...
const showDashboard = (show) => {
  const homepageSections = ['hero-section', 'about-section', 'services-section', 'erp-comparison-section', 'formation-section'];
  sections.forEach(s => {
    if (homepageSections.some(cls => s.classList.contains(cls))) {
      s.style.display = show ? 'none' : 'flex';
      if (s.classList.contains('erp-comparison-section') || s.classList.contains('formation-section') || s.classList.contains('about-section') || s.classList.contains('services-section')) {
